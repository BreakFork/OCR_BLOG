...
class Comment
{
    ...
    //______________________________________________________________________SELECT PENDING COMMENTS BY POST ID (UNPUBLISHED, DESACTIVATED)
     ...
     * @return array $commentsList The list of the unpublished and disactivated comments
     */
    public static function getCommentsPendingListByPost($linkedPost)
    {
        $commentRepository = Database::getEntityManager()->getRepository("Models\\Comment");
        $commentsList = $commentRepository->findBy(
            array(
    ...
    //______________________________________________________________________SELECT DISACTIVATED COMMENTS BY POST ID (DESACTIVATED)

    /**
     * Returns a list of disactivated comments from DB whom were been published
     *
     * @param integer $linkedPost The id of the post whom the comments are attached to
     *
     * @return array $commentsList The list of the disactivated comments
     */
    public static function getDisableCommentsListByPost($linkedPost)
    {
        $commentRepository = Database::getEntityManager()->getRepository("Models\\Comment");
        $commentsList = $commentRepository->findBy(
            array(
    ...
    //_______________________________________________________________________DISABLE COMMENT

    /**
     * Controller method to modify the status of the comment (desactivated and unpublished)
     * Returns a comment from DB selected by id
     *
     * @param int|null $commentId The id of the comment or null if the comment doesn't exist in DB
     *
     * @return Comment|null A comment object from DB, selected by id or null if $commentId is null
     */
    public static function disableComment($commentId): ?Comment
    {
        $commentRepository = Database::getEntityManager()->getRepository("Models\\Comment");
        $comment = $commentRepository->findOneBy(
            array(
